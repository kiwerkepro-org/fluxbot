...
> Entwickelt von [KI-WERKE](https://ki-werke.de) · Gebaut mit Go · Läuft überall
...
1. Lade die passende Datei von der [Releases-Seite](https://github.com/ki-werke/fluxbot/releases/latest) herunter:
...
2. Datei starten (Doppelklick unter Windows; unter Linux/macOS ggf. `chmod +x` ausführen).

3. Der **Einrichtungsassistent** öffnet sich automatisch im Browser – dort trägst du deinen KI-Anbieter, deinen API-Key und deinen Messenger ein. FluxBot startet danach sofort.
...
**Windows (PowerShell):**
```powershell
irm https://fluxbot.ki-werke.de/install.ps1 | iex
```

**Linux / macOS (Terminal):**
```bash
curl -fsSL https://fluxbot.ki-werke.de/install.sh | bash
```
...
Schreib einfach auf Deutsch mit Fluxy – du brauchst keine speziellen Befehle. Aber diese Sätze haben besondere Funktionen:

### Gedächtnis

```
Merke dir, ich wohne in Wien        → Fluxy speichert das dauerhaft
Merk dir: ich bin Grafikdesigner    → Fluxy speichert das dauerhaft
...
Du kannst Fluxy eine eigene Persönlichkeit geben – einfach die Datei `workspace/SOUL.md` (Direktinstallation) bzw. `~/FluxBot/fluxbot-data/SOUL.md` (Docker) bearbeiten:

```markdown
Du bist Fluxy, der KI-Assistent von Mein Unternehmen GmbH.
...
│   ├── SOUL.md                 ← Persönlichkeit von Fluxy (optional)
...
→ Prüfe, ob deine Telegram-User-ID korrekt in `allowFrom` steht ([@userinfobot](https://t.me/userinfobot) hilft dabei).
...
<div align="center">
  <strong>Gebaut mit ❤️ von <a href="https://ki-werke.de">KI-WERKE</a></strong><br>
